...
class Xadrez:
    ...
    def tabuleiro(self):
        ...
        return numpy.rot90(self.__tabuleiro, 3).tolist()  # Torna a posição fácil de ser usada em uma View
    ...
    def __init__(self):
        ...
        self.tornar_posicao_logica = lambda pos: (7 - pos[1], pos[0])  # Torna a posição fácil de ser usada em Xadrez
    ...
    def movimentar_peca(self, old_posicao: tuple, new_posicao: tuple):
        ...
        if peca:  # Se tiver uma peça
            if peca.cor == self.vez:  # Se for a vez dessa peça
                ...
                )  # Ve se é possivel movimentar, se for True a peça considera que tem que movimento aconteceu
                if movimentavel:
    ...
    def get_movimentos(self, linha: int, coluna: int):
        ...
        if peca.cor == self.vez:  # Se a vez permitir a peça mover
            ...
            if isinstance(peca, Peao) and peca.movimentou:  # Se a peça for um Peao
                ultima_jogada = self.jogadas[-1]  # Última jogada é salvo em ultima_jogada
                if not isinstance(ultima_jogada[1], str):  # Se ultima jogada for movimento normal
